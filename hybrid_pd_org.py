...
class HybridPDGenotype(object):
    ...
    def _get_bits_of_summary_mutant(self):
        ...
        if should_increase_memory:
            ...
            new_initial_summary.append(random.choice([True, False]))
            # for i in range(new_number_of_bits_of_summary):
            # temp_decision_list = self.decision_list
            # temp_decision_list.extend([random.choice([True, False]) for i in range(new_number_of_bits_of_summary)])
            # new_decision_list = temp_decision_list
            ...
            if len(new_decision_list) != (2 ** new_number_of_bits_of_memory) * (new_number_of_bits_of_summary + 1):
                ...
                print("NEW DL MATH CHECK: ", (2 ** new_number_of_bits_of_memory) * (new_number_of_bits_of_summary + 1))
                print("LENGTH OF NEW DECISION LIST: ", length_of_new_decision_list)
        ...
        if new_number_of_bits_of_summary > 0:
    ...
    def _get_bits_of_memory_mutant(self):
        ...
        if new_number_of_bits_of_memory > 0:
